  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!this.errors.length) {
      ...
      const [{ insertId }] = await db.execute(
        "INSERT INTO posts (title, body, author, createdDate) VALUES(?, ?, ?, ?)",
...
Post.findByAuthorId = async function (authorId) {
  ...
  let [posts] = await db.execute(
    "SELECT p.title, p.body, p._id, p.author, p.createdDate, u.username, u.avatar FROM posts p JOIN users u ON p.author = u._id WHERE p.author = ? ORDER BY createdDate DESC",
    [authorId]
  );
  return posts;
};
...
Post.search = function (searchTerm) {
  return new Promise(async (resolve, reject) => {
    if (typeof searchTerm == "string") {
      ...
      ===============================================*/
      let [posts] = await db.execute(
        "SELECT p.title, p.body, p._id, p.author, p.createdDate, u.username, u.avatar FROM posts p JOIN users u ON p.author = u._id WHERE MATCH(title, body) AGAINST(?)",
        [searchTerm]
      );
      resolve(posts);
    } else {
...
Post.getFeed = async function (id) {
  ...
  ===============================================*/
  let [posts] = await db.execute(
    "SELECT posts._id, title, createdDate, username, avatar FROM posts JOIN users ON posts.author = users._id WHERE author IN (SELECT followedId FROM follows WHERE authorId = ?) ORDER BY createdDate DESC",
    [id]
  );
